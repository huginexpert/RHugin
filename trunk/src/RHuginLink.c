...
SEXP RHugin_node_set_number_of_states(SEXP Snode, SEXP Sstates)
{
  ...
  RHugin_handle_status_code(h_node_set_number_of_states(node, INTEGER(Sstates)[0]));
...
SEXP RHugin_node_set_state_label(SEXP Snode, SEXP Ss, SEXP Slabel)
{
  ...
  if(LENGTH(Slabel) != n)
    error("Slabel and Ss are not the same length");
  ...
  for(i = 0; i < n; i++) {
    status = h_node_set_state_label(node, (size_t) s[i], (h_string_t) CHAR(STRING_ELT(Slabel, i)));
    if((h_error_t) status != h_error_none) {
      UNPROTECT(2);
      RHugin_handle_error_code((h_error_t) status);
    }
...
SEXP RHugin_node_get_state_index_from_label(SEXP Snode, SEXP Slabel)
{
  SEXP ret = R_NilValue;
  h_error_t error_code = h_error_none;
  int i = 0, *index = NULL, n = LENGTH(Slabel);
  h_node_t node = nodePointerFromSEXP(VECTOR_ELT(Snode, 0));
  ...
  for(i = 0; i < n; i++) {
    index[i] = (int) h_node_get_state_index_from_label(node, (h_string_t) CHAR(STRING_ELT(Slabel, i)));
    error_code = h_error_code();
    if(error_code != h_error_none) {
      UNPROTECT(2);
      RHugin_handle_error_code(error_code);
...
SEXP RHugin_node_set_state_value(SEXP Snode, SEXP Ss, SEXP Svalue)
{
  ...
  double *sv = NULL;
  int i = 0, *s = NULL, n = LENGTH(Svalue);
  h_node_t node = nodePointerFromSEXP(VECTOR_ELT(Snode, 0));
...
SEXP RHugin_node_generate_table(SEXP Snodes)
{
  ...
  for(i = 0; i < n; i++) {
    status = h_node_generate_table(nodePointerFromSEXP(VECTOR_ELT(Snodes, i)));
    RHugin_handle_error();
  }
...
SEXP RHugin_domain_get_marginal(SEXP Sdomain, SEXP Snodes)
{
  ...
  nodes = (h_node_t*) R_alloc(1 + n, sizeof(h_node_t*));
  for(i = 0; i < n; i++)
...
{
  SEXP ret = R_NilValue;
  h_table_t table = h_node_get_distribution(nodePointerFromSEXP(VECTOR_ELT(Snode, 0)));
  RHugin_handle_error();
...
SEXP RHugin_node_get_entered_finding(SEXP Snode, SEXP Sstate)
{
  ...
  finding = REAL(ret);
  for(i = 0; i < n; i++)
    finding[i] = (double) h_node_get_entered_finding(node, (size_t) state[i]);
  UNPROTECT(2);
...
SEXP RHugin_node_get_propagated_finding(SEXP Snode, SEXP Sstate)
{
  ...
  finding = REAL(ret);
  for(i = 0; i < n; i++)
    finding[i] = (double) h_node_get_propagated_finding(node, (size_t) state[i]);
  UNPROTECT(2);
